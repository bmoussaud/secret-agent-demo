# secret-agent-demo-nodejs-service
...
```
tanzu apps workload create secret-agent-demo-nodejs-service -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/secret-agent-demo-nodejs-service-source \
  --type web
```
...
```
tanzu apps workload get secret-agent-demo-nodejs-service
```
