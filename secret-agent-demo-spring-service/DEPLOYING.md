...
```
tanzu apps workload create secret-agent -f config/workload.yaml \
  --local-path . \
  --source-image akseutap4registry.azurecr.io/secret-agent-source \
  --type web
```
...
```
tanzu apps workload get secret-agent
```
